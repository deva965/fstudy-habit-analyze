...
import { api, CreateLogInput } from "./api";
...
export function useStudyLogs() {
  return useQuery({ queryKey: KEYS.logs, queryFn: api.getLogs });
}
...
export function useGoal() {
  return useQuery({ queryKey: KEYS.goal, queryFn: api.getGoal });
}
...
function useInvalidateAll() {
  ...
  return () => {
    ...
    qc.invalidateQueries({ queryKey: KEYS.goal });
  };
}
